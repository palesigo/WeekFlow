...
export function ActivityForm({ date, activity, onClose, startTime='17:00' }: Props) {
 ...
 return <div className="modal-backdrop" onMouseDown={e=>e.target===e.currentTarget&&onClose()}><form className="sheet wide-sheet" onSubmit={submit}>
  ...
  <label>Pessoas<div className="selected-people-summary">{form.personIds.length ? `${form.personIds.length} ${form.personIds.length === 1 ? 'pessoa selecionada' : 'pessoas selecionadas'}` : 'Nenhuma pessoa selecionada'}</div>{people.filter(p=>p.active).length>0&&<div className="chips">{people.filter(p=>p.active).map(p=><button type="button" key={p.id} className={`chip ${form.personIds.includes(p.id)?'selected':''}`} onClick={()=>set('personIds',form.personIds.includes(p.id)?form.personIds.filter(x=>x!==p.id):[...form.personIds,p.id])}>{p.icon??'👤'} {p.name}</button>)}</div>}{people.filter(p=>p.active).length===0&&<div className="people-empty">Ainda não existem pessoas ativas. Cria-as em <b>Definições → Pessoas</b>.</div>}</label>
